...
import pse_gui.Handlers.*;
import pse_gui.Interfaces.IReportingAgentView;
import pse_gui.Models.*;
import pse_gui.Utils.Constants.ItemType;
import pse_gui.Utils.OSValidator;
import pse_gui.Utils.SharedCentralisedClass;
import pse_gui.Utils.UIObjectCreator;

@SuppressWarnings("restriction")
public class MainView implements ChangeListener<Object>, IReportingAgentView {

    @FXML
    BreadCrumbBar<String> breadCrumb;
    @FXML
    VBox leftContainer;
    @FXML
    ...
    Button btnLogs;
    @FXML
    TreeView<ModifiedFile> leftFileTree;
    @FXML
    TreeView<ModifiedFile> rightFileTree;
    @FXML
    BreadCrumbBar<ModifiedFile> leftFileBreadCrumb;
    @FXML
    BreadCrumbBar<ModifiedFile> rightFileBreadCrumb;
    @FXML
    Button btnLeftReset;
    @FXML
    Button btnUpload;
    ...
    Button btnLeftMkdir;
    @FXML
    Button btnRightReset;
    @FXML
    Button btnDownload;
    @FXML
    Button btnDownloadAndOpen;
    @FXML
    Button btnRightMkdir;
    @FXML
    Button btnLeftCancel;
    @FXML
    Button btnRightCancel;
    @FXML
    javafx.scene.control.ProgressBar leftProgressBar;
    @FXML
    javafx.scene.control.ProgressBar rightProgressBar;
    @FXML
    TextField shellTextField;
    @FXML
    Button shellBtnSend;
    @FXML
    SplitPane shellHorizontalSplitPane;
    @FXML
    ScrollPane shellAgentsScollPane;
    @FXML
    VBox shellAgentPane;
    @FXML
    TextArea shellTextArea;
    @FXML
    TextField settingsVerboseLevelTextField;
    @FXML
    TextField settingsRefreshAgentDelayTextField;

    private String backupStyleEnabled;
    private String backupStyleDisabled;

    private LoginView loginController;
    private Stage loginStage = null;

    private MapTreeItem treeRoot;
    private MapTreeItem modulesItem;
    private MapTreeItem agentsItem;
    private MapTreeItem listenersItem;
    private MapTreeItem stagersItem;
    ...
    private ArrayList<TreeItem<ModifiedFile>> leftFileHome = new ArrayList<>();
    private ArrayList<TreeItem<ModifiedFile>> rightFileHome = new ArrayList<>();

    private HashMap<String, ServerResponse> agentsReportingMap = new HashMap<>();
	...
	*/
    ...
    public void initialize() {
        ...
        leftFileBreadCrumb.setOnCrumbAction(event -> {
        ...
        rightFileBreadCrumb.setOnCrumbAction(event -> {
            TreeItem<ModifiedFile> selectedCrumb = event.getSelectedCrumb();
        ...
        settingsVerboseLevelTextField.textProperty().addListener((observable, oldValue, newValue) -> {
            if (!newValue.matches("\\d*")) {
                settingsVerboseLevelTextField.setText(newValue.replaceAll("[^\\d]", ""));
            } else if (!newValue.equals("")) {
                if (Integer.parseInt(settingsVerboseLevelTextField.getText()) < 1
                        || Integer.parseInt(settingsVerboseLevelTextField.getText()) > 3) {
                    settingsVerboseLevelTextField.setText("1");
                }
            }
        });

        settingsRefreshAgentDelayTextField.textProperty().addListener((observable, oldValue, newValue) -> {
            if (!newValue.matches("\\d*")) {
                settingsRefreshAgentDelayTextField.setText(newValue.replaceAll("[^\\d]", ""));
            }
        });
    }

    private MapTreeItem addTreeItem(String title, HashMap<String, Object> value, MapTreeItem parent) {
        return addTreeItemAtPosition(title, value, parent, -1);
    }

    private MapTreeItem addTreeItemAtPosition(String title, HashMap<String, Object> value, MapTreeItem parent, int position) {
        MapTreeItem item;
        MapTreeItem existingItem = getMapTreeItemOfName(title, parent);
    ...
    private MapTreeItem getMapTreeItemOfName(String name, MapTreeItem parent) {
    ...
    private void setupAgentsInShellTab(Map<String, Object> map) {
        shellAgentPane.getChildren().clear();
        ...
        checkBoxAllAgent.setOnAction(event -> {
    ...
    @Override
    public void notifyReportingAgentView(String actualSelectedItemAgentOrListenerToHandle) {
        Platform.runLater(() -> {
            try {
    ...
    public void onBtnConnectClick() {
        try {
            if (loginStage == null) {
                ...
                scene.getStylesheets().add("pse_gui/Views/LoginView.css");
                    ...
                    protected void succeeded() {
                        try {
                            ...
                            if (model.getPowershellEmpireInformations() != null) {
                                ...
                                writeTextToLogArea("Login requested..\n", 1);
                                writeTextToLogArea("user: " + infos.getUserName() + "\n", 2);
                                writeTextToLogArea("host: " + infos.getHost() + "\n", 2);
                                writeTextToLogArea("port: " + infos.getPort() + "\n", 2);
                                if (loginController.isRemote()) {
                                    writeTextToLogArea("SSH enabled..", 2);
                                    writeTextToLogArea("SSH user: " + sshInfos.getUserName() + "\n", 2);
                                    writeTextToLogArea("SSH host: " + sshInfos.getHost() + "\n", 2);
                                    writeTextToLogArea("SSH port: " + sshInfos.getPort() + "\n", 2);
                                }
                            }
                        } catch (Exception e) {
                            SharedCentralisedClass.getInstance().showStackTraceInAlertWindow(e.getMessage(), e);
    ...
    public void writeTextToLogArea(String text, int verboseLevel1to3) {
        if (Integer.parseInt(settingsVerboseLevelTextField.getText()) < verboseLevel1to3)
            return;
    ...
    public void showAlertWindow(AlertType alertType, String messageType, Map<String, Object> map, String key, int verboseLevel1to3) {
        if (Integer.parseInt(settingsVerboseLevelTextField.getText()) < verboseLevel1to3) {
            writeTextToLogArea("An alert occurred. Set a higher verbose level to see alert details", 1);
            return;
        }
    ...
    public void showStackTraceInAlertWindow(String message, String trace, int verboseLevel1to3) {
        if (Integer.parseInt(settingsVerboseLevelTextField.getText()) < verboseLevel1to3) {
            writeTextToLogArea("An exception occurred. Set a higher verbose level to see exception details", 1);
            return;
        }
    ...
    public void onBtnSendClick() {
        writeTextToLogArea("Send command executed..\n", 1);
        ...
        if (this.model.getUserRequest() == null)
            ...
            handler.makeUserRequest(new ListenerRequestResponseHandler(handler, model));
        } else
            handler.makeUserRequest(new UserRequestResponseHandler());
    }
    ...
    public void onBtnResetClick() {
        writeTextToLogArea("Reset Function is undefined!\n", 1);
    }
    ...
    public void onBtnDeleteClick() {
        if (deleteType == ItemType.AGENT) {
            ...
            synchronized (model) {
                ...
                handler.makeUserRequest(new KillAgentResponseHandler(handler, model, actualSelectedItemAgentOrListener));
            }
            ...
            synchronized (model) {
                ...
                handler.makeUserRequest(new DeleteListenerResponseHandler(handler));
            }
        }
    }
    ...
    public void onBtnReportClick() {
        ...
        synchronized (model) {
            ...
            handler.makeUserRequest(new ReportingAgentResponseHandler(agentsReportingMap, this, actualSelectedItemAgentOrListener));
        }
    }
        ...
        for (Node node : agentCheckBoxs) {
            if (((CheckBox) node).isSelected()) {
                ...
                synchronized (model) {
                    ...
                    handler.makeUserRequest(new ShellAgentResponseHandler(handler, model, name, shellTextArea, shellCommand));
                }
            }
        }
    }
